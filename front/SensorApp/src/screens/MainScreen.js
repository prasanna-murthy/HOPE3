...
const ListItem = React.memo(({ item }) => {
  const backgroundColor = item.value > 100 ? '#763626' : 'green';
  ...
  return (
    <View style={[styles.dataItem, { backgroundColor }]}>
      ...
      <Text style={styles.itemText}>
        Status: {item.value > 100 ? 'Exceeds Threshold' : 'Normal'}
      </Text>
    </View>
  );
});
...
export default function MainScreen() {
  ...
  const handleMenuOptionPress = (option) => {
    ...
    if (option === 'Visualize') {
      ...
      navigation.navigate('LedControlScreen'); // Navigate to the LED control screen
    } else if (option === 'FanControl') {
      navigation.navigate('FanControlScreen'); // Navigate to the Fan control screen
    }
  };
  ...
  return (
    <LinearGradient colors={['#FFF2D7', '#FFF2D7', '#FFF2D7']} style={styles.container}>
      ...
      >
        <TouchableOpacity style={styles.modalOverlay} onPress={() => setShowMenu(false)}>
          <View style={styles.menuContainer}>
            ...
            <TouchableOpacity 
              style={[styles.menuItemContainer, { backgroundColor: '#d0d0d0' }]} 
              onPress={() => handleMenuOptionPress('FanControl')}
            >
              <Icon name="flash" size={20} color="black" />
              <Text style={styles.menuItemText}>Fan Control</Text>
            </TouchableOpacity>
          </View>
        </TouchableOpacity>
      </Modal>
      ...
      {!showLedControl ? (
        ...
        <TouchableOpacity
          onPress={handlePress}
          style={[styles.button, { backgroundColor: thresholdExceeded ? '#763626' : 'green' }]}
        >
          <Text style={styles.buttonText}>B LED</Text>
        </TouchableOpacity>
